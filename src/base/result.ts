 ...
 */
export class Result<T = void, D = string, M = unknown> implements IResult<T, D, M> {
     ...
     */
    public static Ok<T, M = unknown, D = string>(data: T, metaData?: M): Result<T, D, M>;
     ...
     * @returns instance of Result.
     */
    public static Ok<T, M = unknown, D = string>(data: T, metaData?: M): IResult<T, D, M>;
     ...
     * @returns instance of Result.
     */
    public static fail<D = string, M = unknown, T = void>(error?: D, metaData?: M): Result<T, D, M>;
     ...
     * @returns instance of Result.
     */
    public static fail<D = string, M = unknown, T = void>(error?: D, metaData?: M): IResult<T, D, M> {
     ...
     */
    public static combine<A = any, B = any, M = any>(
        results: Array<IResult<any, any, any>>,
    ): IResult<A, B, M> {
        const iterator = Result.iterate(results);
        if (iterator.isEmpty())
            return Result.fail('No results provided on combine param' as B) as IResult<A, B, M>;
